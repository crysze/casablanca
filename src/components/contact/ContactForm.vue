<template>
  <div>
    ...
    <form v-if="!emailSent && !error" class="form-group border rounded p-4 mt-4 w-50 mx-auto" ref="form" @submit.prevent="sendEmail">
        ...
        <input class="form-control" type="text" name="user_name" placeholder="Vorname Nachname" required>
        <label class="pt-2">E-Mail</label>
        <input class="form-control" type="email" name="user_email" placeholder="vorname.nachname@gmail.com" required>
        <label class="pt-2">Nachricht</label>
        <textarea class="form-control" name="message" placeholder="Ihre Nachricht an uns" required></textarea>
        <br>
